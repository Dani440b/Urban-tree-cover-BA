...
for i in progress_bar:
    ...
    if np.max(targets[i])==1:
        ...
        sigmoid_mask = torch.nn.functional.softmax(mask_tensor)
        ...
        progress_bar.set_description(f'mask {i+1} has score {scores}')
...
for i in progress_bar:
    ...
    if np.max(targets_eval[i])==1:
        ...
        mask_tensor = torch.from_numpy(combined_mask)
        sigmoid_mask = torch.nn.functional(mask_tensor)
...
save_dir = 'new/citytrees_cache_no_test/patches'
...
save_SAM_patches('new/citytrees_cache_no_test/patches_org', save_dir)
